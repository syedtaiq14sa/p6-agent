  ...
  async readDatabase() {
    ...
    const projects = query(`
      SELECT
        ...
        LAST_RECALC_DATE, ADD_DATE, CREATE_DATE, UPDATE_DATE
      FROM PROJECT
    ...
    const tasks = query(`
      SELECT
        ...
        DRIVING_PATH_FLAG, UPDATE_DATE
      FROM TASK
    `)

    const wbs = query(`
      SELECT
        WBS_ID, PROJ_ID, PARENT_WBS_ID,
        WBS_SHORT_NAME, WBS_NAME, STATUS_CODE,
        ORIG_COST, SEQ_NUM, UPDATE_DATE
      FROM PROJWBS
    `)
    ...
    const resources = query(`
      SELECT
        ...
        RSRC_TYPE, ACTIVE_FLAG, UPDATE_DATE
      FROM RSRC
    `)

    const taskResources = query(`
      SELECT
        TASKRSRC_ID, TASK_ID, PROJ_ID, RSRC_ID,
        TARGET_COST, ACT_REG_COST, ACT_OT_COST,
        REMAIN_COST, TARGET_QTY, ACT_REG_QTY,
        REMAIN_QTY, ACT_START_DATE, ACT_END_DATE,
        TARGET_START_DATE, TARGET_END_DATE, UPDATE_DATE
      FROM TASKRSRC
    `)

    const predecessors = query(`
      SELECT
        TASK_PRED_ID, TASK_ID, PRED_TASK_ID,
        PROJ_ID, PRED_TYPE, LAG_HR_CNT, UPDATE_DATE
      FROM TASKPRED
    `)

    const evmSummary = query(`
      SELECT
        WBS_ID, PROJ_ID,
        BCWP, BCWS, ETC,
        ACT_WORK_COST, REMAIN_WORK_COST,
        ACT_EXPENSE_COST, REMAIN_EXPENSE_COST,
        COMPLETE_CNT, ACTIVE_CNT, NOTSTARTED_CNT,
        TOTAL_DRTN_HR_CNT, REMAIN_DRTN_HR_CNT,
        ACT_START_DATE, ACT_END_DATE,
        BASE_START_DATE, BASE_END_DATE, UPDATE_DATE
      FROM SUMTASK
    `)
